...
for i = 1:length(tspan)
    ...
    if i>1 && flight_data(i,6) <= 0 && ~apogee_flag && flag == 2
        apogee_flag = 1;
        disp(['Apogee at ' num2str(flight_data(i,3)*3.2804) ' ft']);
        disp(['Drogue chute delpoyed at ' num2str(flight_data(i,3)*3.2804) ' ft']);
    elseif i>1 && flight_data(i,6) <= 0 && ~apogee_flag && flag == 1
        apogee_flag = 1;
    end
    ...
    % logic for ending sim when impact is found
    if i>1 && flight_data(i,3) <= 0 && flag == 2
    ...
    if flag == 1  && apogee_flag
        mask = any([true*ones(1,3);flight_data(2:end,4:6) ~= 0],2);
        flight_data = flight_data(mask,:);
        disp(['Apogee at ' num2str(flight_data(end,3)*3.2804) ' ft']);
        break   
    end
end
...
%     thrust_unit = engine_cant_DCM_X*engine_cant_DCM_Z*[-1;0;0];
    ...
    alpha = 0; % angle of attack (deg), 0 since we are just modeling 3DOF
    if mach == 0
        CD = 0;
    else
